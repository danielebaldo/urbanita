...
The map changes hands. CARTO started printing "API KEY REQUIRED" across every tile it served — quietly, with a perfectly valid image underneath — so the basemap is now Esri's Light Gray and Dark Gray Canvas: the same restrained look, still free, still no key, and still a matched pair for the light/dark switch.
